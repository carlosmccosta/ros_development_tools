...
sudo add-apt-repository ppa:thomas-schiex/blender -y # 3d modeling
...
sudo apt-get install libavformat-dev libavcodec-extra -y
...
sudo apt-get install vivacious-unity-gtk-dark vivacious-unity-gtk-light -y
sudo apt-get install vivacious-colors -y
sudo apt-get install vivacious-folder-colors-addon -y
# use unity tweak to change theme to Vivacious-unity-zblackout-green and icons to Vivacious-full-dark-blue-vivid
...
sudo apt-get install nemo nemo-fileroller nemo-terminal nemo-media-columns nemo-gtkhash nemo-compare nemo-seahorse nemo-audio-tab -y
